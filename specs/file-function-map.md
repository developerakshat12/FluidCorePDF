...
| File / Path | Key Symbols & Classes | Primary Responsibilities & Connectivity |
|---|---|---|
| `app/main.cpp` | `main()`, `AppContext`, `SampleNode` | Wave-2 standalone shell (integration.md §1 scoped down): `GtkApplication` → window → `GtkPaned`; left pane hosts `DocumentPane` (PDF path from `argv[1]`), right pane hosts `WorkspaceView`. Wires `Ctrl+S` (`win.save`), `Ctrl+Z` (`win.undo`), and `Ctrl+Shift+Z` / `Ctrl+Y` (`win.redo`) shortcuts and actions. Replaced by the Xournal++ host (`MainWindow::initXournalWidget()` split) in a later wave. |
...
| `app/InkOverlay.h/.cpp` | `class InkOverlay` | Transparent `GtkDrawingArea` overlay on `DocumentPane` capturing pointer/stylus input, streaming through `StrokeStabilizer`, rendering live with wet leading edge and Cairo group alpha isolation, executing eraser hit-testing, dispatching `AddStrokeCommand`/`RemoveStrokeCommand` to `UndoStack`, and providing scoped `invalidateStroke` dirty-rect redraws. |
