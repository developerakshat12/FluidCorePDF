...
| `app/DamageRect.h` | `class DamageRect` | Pure C++20 geometry module calculating pixel-rounded damage bounding boxes for point clicks and stroke segments during vector inking. Tested via `DamageRectTest`. |
| `app/InkOverlay.h/.cpp` | `class InkOverlay` | Transparent `GtkDrawingArea` overlay on `DocumentPane` capturing pointer and stylus input (`GDK_SOURCE_PEN`, mouse), tracking partial damage rectangles via `DamageRect` + `gtk_widget_queue_draw_area` (<= 20ms latency), building stroke geometry, and rendering live in Cairo. |
