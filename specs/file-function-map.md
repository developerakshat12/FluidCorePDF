...
| `app/WorkspaceView.h/.cpp` | `class WorkspaceView` | Standalone right-pane infinite 2D canvas: `GtkDrawingArea` managing 2D affine viewport matrix $\mathbf{M}_{view}$ (smooth focal zoom $5\%$ to $1000\%$, pan gestures, trackpad scrolling), zoom-adaptive infinite dot-grid background, floating interactive minimap HUD with glowing viewport frame, $O(\log N)$ spatial index viewport culling ($<0.05\text{ms}$ query latency across $10^5$ items), interactive `[ ↗ Anchor ]` pill button hit-testing, smooth cubic ease-out camera gliding (`glideToWorldCoord`), card focus aura flash, 150ms zoom-settling debounce, and High-DPI crop surface rendering via `ExcerptTileCache`. |
| `app/PdfDocumentService.h/.cpp` | `class PdfDocumentService`, `using PopplerDocumentPtr`, `using PopplerPagePtr` | Multi-document resolution and concurrency isolation service managing RAII handles with thread-safe `g_object_unref` deleters. Maintains zero-lock main UI thread document handles alongside a dedicated worker document handle synchronized by `m_workerPopplerMutex`. Supports proactive generational cancellation of background rendering requests on document teardown. |
| `app/ExcerptTileCache.h/.cpp` | `class ExcerptTileCache`, `struct CropCacheKey`, `class CairoSurfaceHandle` | 128 MB byte-bounded LRU surface cache rasterizing cropped high-DPI PDF document excerpts across discrete LoD zoom tiers (0.5x, 1.0x, 2.0x, 4.0x, 8.0x) clamped to $1536\text{px}$ ($\approx 9.4\text{MB}$ worst case). Uses `GThreadPool` background workers with generational request IDs and `g_idle_add()` main-thread callbacks. |
| `app/ReturnAnchorPill.h/.cpp` | `class ReturnAnchorPill`, `struct ReturnAnchorPillGeometry` | Interactive floating viewport overlay component in `DocumentPane` (`GtkDrawingArea` in `GtkOverlay`) rendering dark glassmorphism capsule with return icon `↶`, dynamic excerpt badge, close button `✕`, hover pointer feedback, and hit-testing geometry to provide instant return navigation to infinite workspace coordinates. |
