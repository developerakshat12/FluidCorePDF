...
| File / Path | Key Symbols & Classes | Primary Responsibilities & Connectivity |
|---|---|---|
| `app/main.cpp` | `main()`, `AppContext`, `SampleNode` | Wave-2 standalone shell (integration.md §1 scoped down): `GtkApplication` → window → `GtkPaned`; left pane hosts `DocumentPane` (PDF path from `argv[1]`), right pane hosts `WorkspaceView`. Seeds demo nodes through the abstract `FluidCoreAPI` boundary only. Replaced by the Xournal++ host (`MainWindow::initXournalWidget()` split) in a later wave. |
| `app/DocumentPane.h/.cpp` | `class DocumentPane` | Standalone left-pane document viewport: `GtkScrolledWindow` hosting a `GtkDrawingArea` whose Cairo draw pass renders PDF pages in place via `poppler_page_render`, clipped to the exposed region (continuous vertical page column, natural page size). Shows an empty-state/error label when no PDF path is given or loading fails. Rendering only — no business logic in callbacks. |
