...
| `app/DocumentPane.h/.cpp` | `class DocumentPane` | Standalone left-pane document viewport: `GtkScrolledWindow` hosting a `GtkOverlay` containing the Poppler PDF `GtkDrawingArea` and the interactive `InkOverlay`. Auto-loads companion `<file>.xopp` on open; writes companion `<file>.xopp` on close or explicit save (Ctrl+S). Rendering and presentation only — no business logic in callbacks. |
| `app/InkOverlay.h/.cpp` | `class InkOverlay` | Transparent `GtkDrawingArea` overlay on `DocumentPane` capturing pointer and stylus input (`GDK_SOURCE_PEN`, mouse), building stroke geometry (points + pressure + timestamp), rendering live in Cairo during draw with pressure-sensitive line widths and tool alpha, and delegating persistence to `AnnotationStore`. |
