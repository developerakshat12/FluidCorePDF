...
| `libfluidcore/input/PalmRejectionEngine.h/.cpp` | `class PalmRejectionEngine`, `enum class InputDeviceClass`, `enum class InputDecision`, `struct PenEventResult` | Pure C++20 input arbitration state machine: manages Pen, Eraser, Touch, Mouse streams; dynamic hardware profile presets (Wacom, Surface, HP MPP, Generic); contact debounce; proximity tracking; and retroactive touch cancellation delivery. |
...
| `libfluidcore/geometry/StrokeHitTest.h/.cpp` | `class StrokeHitTest`, `struct HitTestResult` | Pure C++20 two-phase vector stroke hit-testing engine: expanded AABB broad-phase rejection, clamped point-to-segment distance narrow-phase evaluation, single-point taps, and nearest-first multi-stroke detection for whole-stroke object erasure. |
