...
int main() {
    try {
        ...
        std::filesystem::remove_all(tempDir, setupEc);
        std::filesystem::create_directories(tempDir, setupEc);
...

        {
            ...
            std::cout
                << "1. Simulating ExcerptDropPayload from crop selection on synthetic PDF...\n";
            ...
            std::filesystem::create_directories(bundledDocsDir, setupEc);
            ...
            if (copyEc) {
                createSyntheticPdf(bundledPath, 15);
            }
            ...
            for (int i = 0; i < 100; ++i) {
                ...
                if (!renderedSurface) {
                    renderedSurface = tileCache.getBestAvailableSurface(
                        bundledPath, 10, payload2.sourceNormalizedRect);
                }
                if (renderedSurface) {
                    break;
                }
        ...
        // Now that engine, tileCache, and docService are fully destructed and threadpools stopped:
        for (int i = 0; i < 10; ++i) {
            g_main_context_iteration(nullptr, FALSE);
        }
        ...
        std::cerr << "Exception in CropDragCrashTest: " << ex.what() << "\n";
        return 1;
    } catch (...) {
        std::cerr << "Unknown exception in CropDragCrashTest\n";
        return 1;
    }
}
