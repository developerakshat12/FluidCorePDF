...
class SqueezeRenderHelper {
  public:
    ...
    // Enforces pixel-snapping and exact boundary continuity: slice[k].screenYEnd == slice[k+1].screenYStart.
    static std::vector<PageSlice> decomposePage(
    ...
    static std::vector<double> extractBreakpoints(
        const std::vector<FluidCore::SqueezeSegment>& segments);
    ...
    static std::vector<FluidCore::Point> subdividePointSpan(
    ...
    static std::vector<FluidCore::Rectangle> subdivideRect(
