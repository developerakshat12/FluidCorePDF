...
GdkPixbuf* renderSvgToPixbuf(const std::string& svgData, int size) {
    GInputStream* stream =
        g_memory_input_stream_new_from_data(svgData.data(), static_cast<gssize>(svgData.size()), nullptr);
    if (!stream) {
        return nullptr;
    }
    ...
    if (!pixbuf) {
        if (err) {
            g_warning("[AppIcons] Failed to render SVG pixbuf (%dx%d): %s", size, size, err->message);
            g_error_free(err);
        }
        return nullptr;
    }
