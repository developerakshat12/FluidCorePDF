...
class WorkspaceSearchEngine {
  public:
    ...
    static std::vector<WorkspaceMatch> search(const WorkspaceModel& model, const std::string& query,
                                              const WorkspaceSearchOptions& options = {});

    // Utility: extracts inline #hashtags from text strings (e.g. "Analysis of #methodology and
    // #P1").
