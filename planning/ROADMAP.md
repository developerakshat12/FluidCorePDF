...
- [x] `ExcerptCardNode` rendering + live re-render on source zoom change *(complete via TASK-3.4: PdfDocumentService multi-document handle isolation and thread-safe Poppler lifecycle, ExcerptTileCache 128 MB byte-bounded LRU tile cache with discrete 0.5x-8.0x LoD tiers clamped to 1536px, asynchronous GThreadPool rasterization with generational cancellation, WorkspaceView 150ms zoom-settling debounce and high-DPI bilinear crop rendering, InkOverlay visual crop tool mode with marquee drag-and-drop, and 20/20 passing CTest suites)*
...
### M4 — Links, Stacks & Search (Weeks 39–48) — EXITED ✅
...
- [x] Workspace Canvas Find & Scoped Search: In-memory search for Workspace cards, excerpts, text, and tags with camera glide/focus *(complete via TASK-4.3: pure C++20 `WorkspaceSearchEngine` full-text search across card snippets, stack titles, explicit tags, and inline hashtags [#tag, tag:xyz]; `ExcerptCardNode` & `CardStackNode` tag storage and mutation; `FluidCoreAPI` searchWorkspace interface; SearchBarWidget scope selector dropdown [Doc, Canvas, All] and adaptive squeeze toggle; WorkspaceView 250ms camera glide navigation and card flash aura pulsing; WorkspaceRenderer glowing amber halos, active match badge pill [Match X/N], and luminous minimap HUD hit dots; DocumentPane cross-scope search coordination; and 26/26 passing CTest suites)*
- [x] Export: annotated PDF flattening + workspace outline to Markdown *(complete via TASK-4.4: pure C++20 `WorkspaceExportEngine` synthesis exporter formatting stack hierarchies [#], excerpt cards [>], visual crop metadata notes, free notes, relational link summaries, and embedded Mermaid flowcharts [```mermaid]; `FluidCoreAPI` exportWorkspaceMarkdown interface; `PdfExportService` Cairo vector PDF flattening pipeline with Poppler vector page rendering and burnt Catmull-Rom/highlighter strokes; GTK3 `ExportDialog` format chooser dialog with Ctrl+E accelerator; and 28/28 passing CTest suites)*
